...
async function getRecommendedUniversities(req, res, next) {
  try {
    const country = normalize(req.query.country || 'USA');

    if (country !== 'usa') {
      return res.status(400).json({
        message: 'University recommendations are currently available for USA only.',
      });
    }

    await universityStore.seedUsaIfEmpty();
    ...
    const universities = await universityStore.findByCountry('USA');
...
async function listUniversities(req, res, next) {
  try {
    ...
    return res.json({ country: country ? country.toUpperCase() : 'ALL', universities });
  } catch (err) {
    return next(err);
  }
}
...
async function updateUniversity(req, res, next) {
  try {
    const university = await universityStore.update(req.params.id, req.body);
